...
//Perm delete accommodation
function deleteAccommodation()
{
  
    let pos = 0;
    //Keep it basic for now, search though, when a match is found delete it. 
    accommodations.savedAccommodation.forEach(place => {
      if (place.name == focusAccommodation)
      {
        //Splice - remove accommodation from list.
        accommodations.savedAccommodation.splice(pos,1);
        saveAccommodationList();
        console.log("Accommmodation Deleted");
      }
      else
      {
